...
float duck_get_velocity(const duck_ptr self) { return self ? self->vx : 0.0f; }
...
bool duck_is_facing_right(const duck_ptr self) { return self ? self->facing_right : false; }

static bool duck_is_shooting(const duck_ptr self) { return self ? self->shooting : false; }

static bool duck_is_alive(const duck_ptr self) { return self ? (!self->dead && self->health > 0) : false; }

int duck_get_health(const duck_ptr self) { return self ? self->health : 0; }

bool duck_is_within_bounds(const duck_ptr self) {
    if (!self)
        return false;

    return (self->x >= self->bounds_min_x && self->x + DUCK_WIDTH <= self->bounds_max_x);
}
